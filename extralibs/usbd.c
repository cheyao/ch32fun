...
// TODO: Seperate HP and LP interrupts?
...
{
	static uint8_t zeros[] = { 0, 0 }; // Small macro to help reduce size
	const uint32_t istr = USBD->ISTR;
	int len = 0;

	if ( 5 == ( istr & USBD_EP_ID ) )
	{
		funDigitalWrite( PA0, 1 );
		Delay_Us( 5 );
		funDigitalWrite( PA0, 0 );
		Delay_Us( 5 );
	}
	...
	if ( istr & USBD_CTR )
	{
		...
		{
			USBD_DEBUGPRINTF( "Both RX & TX! Processing is too slow\n" );
		}
#endif
		...
		if ( epr & USBD_CTR_RX )
		{
			...
			if ( ep == 0 && epr & USBD_SETUP )
			{
				...
				// 32 bits)
				// What a weird design
				const uint8_t USBD_request_type = USBDCTX.USBD_SetupReqType = USBD_EP[0][0] & 0xFF;
				const uint8_t USBD_request = USBDCTX.USBD_SetupReqCode = USBD_EP[0][0] >> 8;
				const uint32_t USBD_index = ( (uint32_t)USBD_EP[0][2] );
				const uint32_t USBD_indexValue = USBDCTX.USBD_IndexValue =
					( ( (uint32_t)USBD_EP[0][2] ) << 16 ) | USBD_EP[0][1];
				const uint16_t USBD_length = USBDCTX.USBD_SetupReqLen = USBD_EP[0][3];

				// Copy to CTRL0BUFF
				// Can be removed if CTRL0BUFF unused
				for ( int i = 0; i < USBD_length; ++i )
				{
					CTRL0BUFF[i] = ( USBD_EP[0][i / 2] >> ( ( i & 1 ) << 3 ) ) & 0xFF;
				}
					...
					case USB_CLEAR_FEATURE:
						...
						if( ( USBD_request_type & USB_REQ_RECIP_MASK ) == USB_REQ_RECIP_ENDP )
						{
							if( (uint8_t)( USBD_indexValue & 0xFF ) == USB_REQ_FEAT_ENDP_HALT )
							{
								...
								{
									if( USBD_index & DEF_UEP_IN  && (USBDCTX.endpoints[ep].mode & USBD_EP_MODE_TX) ) 
				SetEPR_Status( ep, USBD_EPR_STAT_TX_MASK, USBD_EPR_STAT_TX_NAK );
									else if( USBD_index & DEF_UEP_OUT && (USBDCTX.endpoints[ep].mode & USBD_EP_MODE_RX) ) 
				...
				SetEPR_Status( ep, USBD_EPR_STAT_RX_MASK, USBD_EPR_STAT_RX_STALL );
						...
						default:
							...
							USBD_DEBUGPRINTF( "EP0 SETUP UNHANDLED: %d\n", USBD_request );
							break;
					}
				}
				else
				{
					...
					{
						buff[i] = ( USBD_EP[ep][i / 2] >> ( ( i & 1 ) << 3 ) ) & 0xFF;
					}
#endif
		...
		if ( ep == 0 )
		{
			...
			else
			{
				...
				{
					USBD_EP[0][i / 2] = *( (const uint16_t *)( USBDCTX.pCtrlPayloadPtr + i ) );
				}
		...
		if ( epr & USBD_CTR_TX )
		{
			...
			else
			{
				...
				{
					SetEPR_Status( 0, USBD_EPR_STAT_RX_MASK, USBD_EPR_STAT_RX_NAK );
				}
			}
	...
	if ( istr & USBD_RESET )
	{
		USBD_DEBUGPRINTF( "\033[93mUSB RESET\033[0m\n" );
	...
	{
		USBD_DEBUGPRINTF( "ERROR! USB error\n" );
...
int USBD_SendEndpoint( int endp, int len )
{
	...
	USBDCTX.endpoints[endp].len = len;

	for ( int i = 0; i < len; i += 2 )
	{
		USBD_EP[endp][i / 2] = *( (const uint16_t *)( EP_buffer + i ) );
	}
...
int USBD_SendEndpointNEW( int endp, uint8_t *data, int len, int copy )
{
	...
	// Will be setup in main int loop
	USBDCTX.endpoints[endp].in = data;
	USBDCTX.endpoints[endp].len = len;

	for ( int i = 0; i + 1 < len; i += 2 )
	{
		USBD_EP[endp][i / 2] = data[i] | ( (uint16_t)data[i + 1] << 8 );
	}
	if ( len % 2 != 0 )
	{
		USBD_EP[endp][len / 2] = data[len - 1];
	}
